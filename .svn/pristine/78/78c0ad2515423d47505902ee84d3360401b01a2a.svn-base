...
final class Compressor extends CompBase {
    ...
    public void compress() {
        ...
        next_byte: while ((c = input.readByte()) != -1) {
            ...
            Timer.getInstance().getWatch("output").resume();
            output(ent);
            Timer.getInstance().getWatch("output").suspend();
            outCount++;
            ent = c;
...
final class Decompressor extends CompBase {
    ...
    public void decompress() {
        ...
        Timer.getInstance().setTime("decompress", System.currentTimeMillis() - startTime);
    }
